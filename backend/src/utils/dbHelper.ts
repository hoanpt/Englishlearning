...
export const saveAstronaut = async (astronautData: any): Promise<any> => {
  if (mongoose.connection.readyState === 1) {
    ...
    const updated = {
      ...
      manuallyUnlockedPreStarter: astronautData.manuallyUnlockedPreStarter || existing.manuallyUnlockedPreStarter || [],
      manuallyUnlockedPET: astronautData.manuallyUnlockedPET || existing.manuallyUnlockedPET || [],
